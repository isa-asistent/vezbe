...
public class GreetingController {
	 ...
	 */
	@Operation(description = "Returns greeting by id", method="GET")
	@ApiResponses(value = {
	...
	public ResponseEntity<Greeting> updateGreeting(@RequestBody GreetingDTO greeting, @PathVariable long id) {
		try {
			Greeting updatedGreeting = greetingService.update(greeting, id);
	...
	@Operation(description = "Updates text of a greeting")
	@ApiResponses(value = {
			@ApiResponse(responseCode = "200", description = "Text successfully updated",
					content = { @Content(mediaType = "application/json", schema = @Schema(implementation = Greeting.class)) }
			),
			@ApiResponse(responseCode = "404", description = "Greeting not found", content = @Content)})
	@PatchMapping (value = "/{id}", consumes = MediaType.APPLICATION_JSON_VALUE, produces = MediaType.APPLICATION_JSON_VALUE)
	public ResponseEntity<Greeting> updateGreetingText(@RequestBody GreetingTextDTO greetingDTO,
													   @Parameter(description = "Greeting id", required = true) @PathVariable long id) {
	 ...
	 */
	@Operation(description = "Deletes a greeting", method = "DELETE")
	...
	public ResponseEntity<Greeting> deleteGreeting(@Parameter(description = "Greeting id", required = true) @PathVariable("id") Long id) {
	 ...
	 */
	@Operation(description = "Searches for greetings that contain specific text", method = "GET")
